...
export async function addStatisticGame(resultsSprint: SprintResult[], gameName: string){
    ...
    if(user){
        ...
        for(const i of resultsSprint){
            if(i.result){
                thisSeries +=1;
                await requestMethods().createUserWord(user.id, i.wordID, 'learned', user.token);
            }else{
                allSeries.push(thisSeries);
                thisSeries = 0
                await requestMethods().createUserWord(user.id, i.wordID, 'complicated', user.token)
            }
        }
